...
// Reads the opt-in coreVerdict/coreCriticalFlags fields
// /api/market/trend-screen?...&withDecision=1 attaches server-side — the
// same real am-core-engine.js verdict now driving the Workspace Decision
// banner and Scanner grade (buildEvFromRow -> computeEntryPlan ->
// computeRedFlags -> computeCoreScore -> classifyCoreVerdict). One Engine
// Migration Phase 6 (2026-08-23): was row.unifiedDecision/
// unifiedCriticalFlags off the now-retired classifyDeepScanDecision, which
// disagreed with the on-screen verdict for the same symbol. NOT
// computeNextAction above (that one still reads the old row.verdict/
// atBuyPoint and has its own much wider fan-out — deliberately untouched
// this phase). The literal verdict list mirrors watchlist-setup-alerts.js's
// server-side ACTIONABLE_DECISIONS Set — no client-reachable export of
// that exists (CJS/ESM split, same as this file's other cross-runtime
// constants).
export function isUnifiedGo(row) {
  return row?.coreCriticalFlags === 0 &&
    ["EARLY_BUY", "BUY"].includes(row?.coreVerdict);
}
