...
export function summarizeBuyPrice(priceToPay, verdict) {
  ...
  const { current, idealEntryLow, idealEntryHigh, breakoutEntry, target, rr } = priceToPay;
  const rrTxt = rr != null ? ` · ${rr.toFixed(1)}:1 reward/risk` : "";
  const targetTxt = target != null ? ` toward $${target}` : "";

  if (verdict?.verdict === "AVOID") {
    return { label: "NOT A BUY RIGHT NOW", ok: false, reason: verdict.reason };
  }
  if (verdict?.verdict === "OVEREXTENDED") {
    return { label: `WAIT FOR $${idealEntryLow} – $${idealEntryHigh}`, ok: false, reason: "Price already ran — buying here has poor reward/risk. Let it come back to the real buy zone first." };
  }
  const inZone = Number.isFinite(current) && current >= idealEntryLow && current <= breakoutEntry * 1.03;
  if (inZone && verdict?.verdict === "BUY ZONE") {
    return { label: `$${current.toFixed(2)} (right now)`, ok: true, reason: `In the real buy zone${targetTxt}${rrTxt}.` };
  }
  return { label: `$${idealEntryLow} – $${idealEntryHigh}`, ok: true, reason: `Real breakout buy zone (pivot $${breakoutEntry})${targetTxt}${rrTxt} — not there yet.` };
}
