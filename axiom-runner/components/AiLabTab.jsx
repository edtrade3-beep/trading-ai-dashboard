        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ ...card({ padding: "10px 16px" }) }}>
              <div style={{ fontFamily: MONO, fontSize: 15, fontWeight: 900, color: C.accent, marginBottom: 10 }}>🤖 AI LABORATORY</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {sections.map(s => (
                  <button key={s.id} onClick={() => setAilabSection(s.id)}
                    style={{ fontFamily: MONO, fontSize: 12, fontWeight: ailabSection === s.id ? 800 : 500, background: ailabSection === s.id ? `${C.accent}22` : C.surface, border: `1px solid ${ailabSection === s.id ? C.accent : C.border}`, color: ailabSection === s.id ? C.accent : C.textDim, borderRadius: 5, padding: "7px 14px", cursor: "pointer" }}>{s.label}</button>
                ))}
              </div>
            </div>
            ...
            {ailabSection === "pattern" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ ...card({ padding: 16 }), display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: C.accent }}>AI CHART PATTERN RECOGNIZER</div>
                  ...
                  <button onClick={() => patternInput.trim() && fetchAIPattern(patternInput.trim())} disabled={patternLoading}
                    style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, background: patternLoading ? C.surface : C.accent, border: "none", color: patternLoading ? C.textDim : "#fff", borderRadius: 6, padding: "9px 16px", cursor: patternLoading ? "default" : "pointer" }}>
                ...
                {patternResult && !patternLoading && (
                  <div style={{ ...card({ padding: 20, borderLeft: `4px solid ${C.accent}` }) }}>
                    <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 800, color: C.accent, marginBottom: 10 }}>PATTERN ANALYSIS — {patternTicker}</div>
                    {patternResult.error ? (
            ...
            {ailabSection === "scenario" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ ...card({ padding: 16 }) }}>
                  <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: C.accent, marginBottom: 10 }}>MACRO SCENARIO PLANNER</div>
                  ...
                  <button onClick={() => fetchMacroScenario(scenarioInput)} disabled={scenarioLoading || !scenarioInput.trim()}
                    style={{ marginTop: 10, fontFamily: MONO, fontSize: 12, fontWeight: 700, background: scenarioLoading || !scenarioInput.trim() ? C.surface : C.accent, border: "none", color: scenarioLoading ? C.textDim : "#fff", borderRadius: 6, padding: "10px 20px", cursor: "pointer" }}>
                ...
                {scenarioResult && !scenarioLoading && (
                  <div style={{ ...card({ padding: 20, borderLeft: `4px solid ${C.accent}` }) }}>
                    <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 800, color: C.accent, marginBottom: 10 }}>SCENARIO IMPACT</div>
            ...
            {ailabSection === "earnings" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ ...card({ padding: 16 }) }}>
                  <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: C.accent, marginBottom: 10 }}>EARNINGS CALL SUMMARIZER</div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                    ...
                    <button onClick={summarizeEarningsCall} disabled={earningsCallLoad || !earningsCallText.trim()}
                      style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, background: earningsCallLoad || !earningsCallText.trim() ? C.surface : C.accent, border: "none", color: earningsCallLoad ? C.textDim : "#fff", borderRadius: 6, padding: "10px 20px", cursor: "pointer" }}>
                ...
                {earningsCallResult && !earningsCallLoad && (
                  <div style={{ ...card({ padding: 20, borderLeft: `4px solid ${C.accent}` }) }}>
                    <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 800, color: C.accent, marginBottom: 10 }}>SUMMARY</div>
            ...
            {ailabSection === "recap" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ ...card({ padding: 20 }), display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                  <div>
                    <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: C.accent }}>SESSION RECAP GENERATOR</div>
                  ...
                  <button onClick={generateSessionRecap} disabled={sessionRecapLoad}
                    style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, background: sessionRecapLoad ? C.surface : C.accent, border: "none", color: sessionRecapLoad ? C.textDim : "#fff", borderRadius: 6, padding: "10px 20px", cursor: "pointer" }}>
                ...
                {sessionRecapResult && !sessionRecapLoad && (
                  <div style={{ ...card({ padding: 20, borderLeft: `4px solid ${C.accent}` }) }}>
                    <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 800, color: C.accent, marginBottom: 10 }}>TODAY'S SESSION RECAP</div>
            ...
            {ailabSection === "checklist" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ ...card({ padding: 16 }), display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: C.accent }}>PRE-TRADE CHECKLIST</div>
                  <div>
