...
// initStoryAssetStore() (2026-09-09, real fix for Story AI's generated
// images/audio/video vanishing between pipeline steps — Render's own
// local disk was never actually persisting them; same real Postgres-
// bytea pattern initPhotoStore already proved out for dealer vehicle
// photos) follows the same shared-pool sequencing.
initPgStore()
  .then(() => initPhotoStore())
...
server.listen(PORT, HOST, () => {
  ...
  setInterval(() => {
    ...
    // Opportunity Digest — hourly, 10am-3pm ET (2026-09-16, explicit
    // request: "I want opportunities come to me not search for it"). A
    // real, guaranteed push on a fixed schedule, distinct from the
    // 15-min Opportunity Hunter Alerts job (registerJob below), which
    // only fires on an actual state-change event and could otherwise
    // stay silent for hours on a quiet day. Six real dispatches/day
    // (10/11/12/1/2/3), each a fresh top-10 read — never the same stale
    // scan replayed.
    for (const dh of [10, 11, 12, 13, 14, 15]) {
      if (h === dh && m < 6) {
        const key = `${today}-${dh}`;
        if (_oppDigestSentKey !== key) { _oppDigestSentKey = key; require("./src/opportunity-hunter-alerts").sendOpportunityDigest().catch(() => {}); }
      }
    }
