...
function App() {
  ...
  const pbSiteLinks = (v) => {
    ...
    return [
      ["CG", "CarGurus", `https://www.cargurus.com/Cars/l-Used-${mk}-${md}#resultsPage=1&zip=45014&distance=200`],
      ["Cars", "Cars.com", `https://www.cars.com/shopping/results/?stock_type=used&makes[]=${mk}&models[]=${mk}-${md}&year_min=${yr}&year_max=${yr}&maximum_distance=200&zip=45014`],
      ["TC", "TrueCar", `https://www.truecar.com/used-cars-for-sale/listings/${mk}/${md}/?zipcode=45014&searchRadius=200`],
      ["AT", "AutoTrader", `https://www.autotrader.com/cars-for-sale/all-cars/${mk}/${md}/fairfield-oh-45014?searchRadius=200&startYear=${yr}&endYear=${yr}`],
      ["CFS", "CarsForSale", g("carsforsale.com")],
      ["CFX", "Carfax", g("carfax.com")],
    ];
  };
  ...
  const savePbKey = async (provider, value) => {
    ...
    try {
      ...
      const lbl = { brave: "Brave", marketcheck: "MarketCheck" }[provider];
      showToast(`${lbl} key saved — scanning enabled`, "success");
    } catch (e) { showToast(e.message, "error"); }
  };
  const pbAddVehicle = () => {
  ...
  const pbScanVehicle = async (v) => {
    if (pbKeyConfigured === false) { showToast("Add a MarketCheck or Brave key first (box at the top)", "error"); return; }
  ...
  const pbScanAll = async () => {
    ...
    if (pbKeyConfigured === false) { showToast("Add a MarketCheck or Brave key first (box at the top)", "error"); setPbStatus("⚠️ Add a MarketCheck or Brave key above to enable scanning."); return; }
            ...
            {tab === "Price Beater" && (() => {
              ...
              return (
                <Panel title="Price Beater" badge="Real dealer listings" styles={styles}>
                  <div style={{ fontSize: 13, color: styles.smallLabel.color, marginBottom: 12 }}>
                    For each car, finds the <b>cheapest competing dealer</b> near <b>45014</b> — shows the dealer, how far, the odometer, and a link to the listing. Your own listings are excluded.
                  </div>
                          ...
                          {[...inventory].sort(pbSortFn).map(v => {
                            ...
                            return (
                              <tr key={v.vin} style={{ borderTop: "1px solid #eef1f4" }}>
                                ...
                                <td style={{ padding: "8px 10px", fontWeight: 600 }}>{c ? (c.dealer || c.source || "dealer") : (r.scanned ? "no comps in 200 mi" : "—")}</td>
                                ...
                                <td style={{ padding: "8px 10px", whiteSpace: "nowrap" }}>{pbSiteLinks(v).map(([abbr, name, url]) => (
                                  <a key={abbr} href={url} target="_blank" rel="noopener" title={`Search ${name}`} style={{ fontSize: 10, marginRight: 4, padding: "2px 5px", border: `1px solid ${styles.card.borderColor || "#ddd"}`, borderRadius: 4, color: styles.buttonPrimary.background, textDecoration: "none" }}>{abbr}</a>
                                ))}</td>
