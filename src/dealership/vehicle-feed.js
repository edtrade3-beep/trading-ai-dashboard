...
//
// KNOWN GAP: `images` is left blank. There is no server-side photo storage in
// this app today (the Photos tab processes images client-side only, nothing is
// uploaded/persisted), and Meta requires at least one real, publicly reachable
// image URL per vehicle. Until photos are hosted somewhere durable and public,
// this feed will not pass Meta's catalog validation as-is.
...
function buildVehicleFeedCsv(baseUrl) {
  ...
  const rows = inv.filter(feedEligible).map(v => {
    const rec = {
      ...
      images: "", // real gap — see file header
