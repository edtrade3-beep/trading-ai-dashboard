...
const CATALYST_RULES = [
  ...
  // Crypto-native catalysts (2026-09-07, Why-Is-It-Moving extended to
  // crypto) — additive only, matched by keyword like every other rule
  // here, so equity headlines are completely unaffected. Without these,
  // real high-impact crypto news (an ETF approval, an exchange hack, an
  // SEC enforcement action) fell into the generic OTHER bucket (weight
  // 30), which would almost never clear rankMoveDrivers' 60-point
  // impact-score bar — silently making the "why is it moving" answer for
  // crypto always "unexplained" even when a real, obvious catalyst existed.
  { category: "CRYPTO_REGULATORY", weight: 85, keywords: ["bitcoin etf", "spot bitcoin etf", "spot ether etf", "etf approval", "etf approved", "etf rejected", "sec approves", "sec sues", "sec charges", "cftc charges", "crypto regulation", "regulatory crackdown", "banned crypto", "crypto ban", "stablecoin bill", "market structure bill"] },
  { category: "CRYPTO_SECURITY", weight: 80, keywords: ["exchange hacked", "hack exploit", "exploited for", "bridge hack", "wallet drained", "rug pull", "smart contract exploit", "exchange insolvent", "files for bankruptcy"] },
  { category: "CRYPTO_NETWORK", weight: 55, keywords: ["hard fork", "network upgrade", "mainnet launch", "halving event", "the halving"] },
