...
// The one real fetch+compute path, shared by the HTTP route below AND by
// tournament-engine.js's Top-25 valuation enrichment (2026-09-17 follow-up,
// "Tournament + Top Opportunities valuation integration") — reusing this
// exact function (not a second fetch/compute pass) is what keeps that a
// real ONE-ENGINE consumer instead of a duplicate valuation path, and the
// shared 15-min cache is what keeps the tournament tick's repeated calls
// for the same still-ranked symbols cheap.
async function getValuationProfile(symbol, keys) {
  ...
  if (!fundamentals) return { ok: false, error: `No real fundamentals data for ${symbol} right now.` };
...
async function handleValuation(req, res, requestUrl) {
  ...
  try {
    ...
    const data = await getValuationProfile(symbol, keys);
    return writeJson(res, 200, { symbol, ...data });
  } catch (err) {
    return writeJson(res, 200, { ok: false, error: err instanceof Error ? err.message : "Valuation unavailable." });
  }
}

module.exports = { handleValuation, getValuationProfile };
