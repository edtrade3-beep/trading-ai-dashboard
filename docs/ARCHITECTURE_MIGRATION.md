...
- [~] Portfolio consumes canonical decisions and exposes factor/event concentration (open-position edge monitor carries canonical `assetDecision`; factor/sector concentration is DONE — `portfolio-correlation-calc.js` + `GET /api/ai-hub/portfolio-correlation`, verified 2026-09-10, doc previously undersold this; event concentration — e.g. multiple open positions all facing earnings the same week — is the genuinely remaining half)
