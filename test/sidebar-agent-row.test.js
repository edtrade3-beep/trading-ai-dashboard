...
// the Agent tab has a permanent, visible sidebar row (2026-09-13,
...
// since before the 2026-09-05 nav consolidation — "dont see it"). Relabeled
// "AI Agent" -> "Agent" (2026-09-15, AI-Trade-Desk-restructure master
// prompt's exact target label) — same real row/activeTab, so this test's
// own intent (a permanent row exists) still holds, just checking the new
// label. Same fs.readFileSync + regex structural-check convention as
...
ok("SIDEBAR_ITEMS includes a real 'agent' entry pointing at activeTab \"agent\"", () => {
  ...
  assert.match(block, /\{ id: "agent", label: "Agent", icon: "[^"]+", tab: "agent" \}/);
});
