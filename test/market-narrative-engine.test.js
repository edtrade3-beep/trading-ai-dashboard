...
// داير السوق اليوم"). Covers renderMarketNarrativeText only — buildMarket
// Narrative itself does real internal HTTP fetches against a running
// server, same convention as morning-mode-engine.js/deep-scan-engine.js's
// own build* functions (exercised live, not unit-tested).
...
console.log("Checking renderMarketNarrativeText — real, honest, never-fabricated market narrative…");

ok("no real data at all renders honest empty-state lines, never fabricated movers", () => {
  const text = renderMarketNarrativeText(base);
  assert.ok(text.includes("غير متوفر حالياً"));
  assert.ok(text.includes("لا توجد حركة صاعدة"));
  assert.ok(text.includes("لا توجد حركة هابطة"));
});
...
ok("a real BUY-family mover shows a real buy point/stop/target, never a fabricated sell point", () => {
  ...
  assert.ok(text.includes("اختراق نطاق الافتتاح (Breakout)"));
  assert.ok(text.includes("Bull BOS"));
  assert.ok(text.includes("STRONG_BUY (شراء)"));
  assert.ok(text.includes("نقطة الشراء: 118"));
  assert.ok(!text.includes("لا توجد نقطة بيع"));
});
...
ok("a real AVOID/EXIT-family decliner shows the real stop level with an honest disclosure — never a fabricated short-entry price", () => {
  ...
  assert.ok(text.includes("AVOID (بيع/تجنب)"));
  assert.ok(text.includes("وقف الخسارة الحقيقي 88"));
  assert.ok(text.includes("لا توجد نقطة بيع على المكشوف حقيقية"));
...
ok("a mover with no real verdict says so honestly, never inventing one", () => {
  ...
  const text = renderMarketNarrativeText({ ...base, moversUp: [mover] });
  assert.ok(text.includes("لا يوجد فيردكت حقيقي متاح"));
});
...
ok("real breakouts list renders with real RVOL, separate from the up/down mover lists", () => {
  ...
  assert.ok(text.includes("اختراقات حقيقية لنطاق الافتتاح"));
